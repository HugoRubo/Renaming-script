...
DEFAULT_SUIVI_PATH = Path(r"C:\Users\HugoCHASTENET\OneDrive - JICAP\Documents\Python scripts\Rename Invoice + insert excel line.xlsx")
EXCEL_HEADERS = ["Site", "Prestataire", "Facture", "Montant HT", "Montant TTC", "Date insertion"]
...
def _normalize_amount_string(amount: Optional[str]) -> Optional[str]:
    """Normalize amount strings to French format with comma separator."""
    if not amount:
        return None
    ...
    normalized = cleaned_no_space.replace(",", ".")
...
def append_row_to_excel(suivi_path, site, prestataire, facture, montant_ht, montant_ttc, date_insertion=None):
    """Ajoute une ligne au classeur Excel en évitant les doublons (Site, Facture). Créé le fichier si nécessaire."""
    ...
    normalized_facture = (facture or "").strip().lower()

    if normalized_site or normalized_facture:
        for row in worksheet.iter_rows(min_row=2, values_only=True):
            existing_site = (row[0] or "").strip().lower()
            existing_facture = (row[2] or "").strip().lower()
            if existing_site == normalized_site and existing_facture == normalized_facture:
    ...
    worksheet.append([
        ...
        prestataire or "",
        facture or "",
        _normalize_amount_string(montant_ht) or "",
        _normalize_amount_string(montant_ttc) or "",
        date_insertion,
    ])
...
class PDFRenamer:
    ...
    def _load_prestataires_data(self) -> Dict[str, List[str]]:
        ...
        logger.info(f"Auto-detected delimiter: '{delimiter}' for Prestataires.csv")
        ...
        with open(csv_path, 'r', encoding='utf-8-sig') as file:  # Handle BOM
            ...
            if 'Collecte' not in reader.fieldnames or 'Combinations' not in reader.fieldnames:
                raise ValueError(f"CSV file must contain 'Collecte' and 'Combinations' columns. Found: {list(reader.fieldnames)}")
    ...
    def _load_valid_collectors(self) -> set:
        ...
        try:
            ...
            with open(csv_path, 'r', encoding='utf-8-sig') as file:
                ...
                # Verify we have the required columns
                if 'Collecte' not in reader.fieldnames:
                    raise ValueError(f"CSV file must contain 'Collecte' column. Found: {list(reader.fieldnames)}")
    ...
    def _prepare_tracking_entry(self, pdf_path: Path, extracted_data: Dict, new_filename: Optional[str]) -> Dict[str, Optional[str]]:
        """Prepare the data required for Excel tracking."""
        ...
        metadata_cache = None
        metadata_amounts = (None, None)
        text_amounts = (None, None)
        text_loaded = False

        def get_metadata():
            nonlocal metadata_cache, metadata_amounts
            if metadata_cache is None:
                metadata_cache = self._read_pdf_metadata(pdf_path)
                metadata_amounts = self._extract_amounts_from_metadata(metadata_cache)
            return metadata_cache

        def metadata_value(keys: List[str]) -> Optional[str]:
        ...
        def metadata_amount(index: int) -> Optional[str]:
            get_metadata()
            return metadata_amounts[index]

        def ensure_text_amounts() -> Tuple[Optional[str], Optional[str]]:
            nonlocal text_loaded, text_amounts
            if not text_loaded:
                text_loaded = True
                pdf_text = self._extract_pdf_text(pdf_path)
                text_amounts = self._extract_amounts_from_text(pdf_text)
            return text_amounts

        filename_to_parse = Path(new_filename).stem if new_filename else pdf_path.stem
        ...
        if not site:
            site = _first_non_empty(metadata_value(['site', 'site_number', 'code client', 'code_client']))
        if not site:
            site = self._extract_site_from_filename(filename_to_parse)
        if not site:
            site = self._extract_site_from_filename(pdf_path.stem)
        ...
        if not prestataire:
            prestataire = _first_non_empty(metadata_value(['prestataire', 'invoice_provider', 'provider', 'collecte']))
        if not prestataire:
            prestataire = self._extract_prestataire_from_filename(filename_to_parse)
        if not prestataire:
            prestataire = self._extract_prestataire_from_filename(pdf_path.stem)

        facture = _first_non_empty(
            extracted.get('invoice_number'),
            raw_analysis.get('invoice_number')
        )
        if not facture:
            facture = _first_non_empty(metadata_value(['facture', 'invoice_number', 'numero_facture', 'invoice']))
        if not facture:
            facture = self._extract_invoice_number_from_filename(filename_to_parse)
        if not facture:
            facture = self._extract_invoice_number_from_filename(pdf_path.stem)

        montant_ht = _first_non_empty(
            extracted.get('montant_ht'),
            raw_analysis.get('montant_ht')
        )
        if not montant_ht:
            montant_ht = _first_non_empty(metadata_amount(0))

        montant_ttc = _first_non_empty(
            extracted.get('montant_ttc'),
            raw_analysis.get('montant_ttc')
        )
        if not montant_ttc:
            montant_ttc = _first_non_empty(metadata_amount(1))

        if not montant_ht or not montant_ttc:
            ht_text, ttc_text = ensure_text_amounts()
            if not montant_ht:
                montant_ht = _first_non_empty(ht_text)
            if not montant_ttc:
                montant_ttc = _first_non_empty(ttc_text)

        if not montant_ht:
            montant_ht = self._extract_amount_from_filename(filename_to_parse, 'ht')
        if not montant_ttc:
            montant_ttc = self._extract_amount_from_filename(filename_to_parse, 'ttc')

        return {
            'site': site or '',
            'prestataire': prestataire or '',
            'facture': facture or '',
            'montant_ht': montant_ht or '',
            'montant_ttc': montant_ttc or '',
            'date_insertion': None,
        }
    ...
    def rename_pdfs_in_directory(self, directory: Path, dry_run: bool = True) -> Dict:
        ...
        for i, pdf_file in enumerate(pdf_files, 1):
            ...
            try:
                ...
                try:
                    tracking_entry = self._prepare_tracking_entry(pdf_file, extracted_data, new_filename)
                    append_row_to_excel(
                        self.suivi_excel_path,
                        tracking_entry['site'],
                        tracking_entry['prestataire'],
                        tracking_entry['facture'],
                        tracking_entry['montant_ht'],
                        tracking_entry['montant_ttc'],
                        tracking_entry['date_insertion'],
                    )
